      ...
      <div className="min-h-screen flex items-center justify-center p-4 text-center">
        <div className="bg-white p-8 rounded-2xl shadow-sm max-w-md w-full">
          ...
          <p className="text-gray-500 mt-2">
            Este quiz não existe ou não está publicado.
          </p>
        </div>
      </div>
    );
          ...
          <AnimatePresence mode="wait">
                ...
                {(!currentQ.type || currentQ.type === "choice") && (
                  <div className="space-y-3">
                    {currentQ.options.map((opt: any) => {
                      const isSelected = responses[currentQ.id]?.id === opt.id;
                      return (
                        <button
                          key={opt.id}
                ...
                {currentQ.type === "scale" && currentQ.scaleConfig && (
                  <div className="mt-8">
                    <div className="flex justify-between text-sm font-semibold opacity-70 mb-6 px-1">
                      <span>{currentQ.scaleConfig.minLabel}</span>
                      <span>{currentQ.scaleConfig.maxLabel}</span>
                    </div>
                    ...
                    <div className="flex justify-between gap-2 sm:gap-3">
                      {Array.from(
                        {
                          length:
                            currentQ.scaleConfig.max -
                            currentQ.scaleConfig.min +
                            1,
                        },
                        (_, i) => currentQ.scaleConfig.min + i,
                      ).map((val) => {
                        const isSelected =
                          responses[currentQ.id]?.points === val;
                        return (
                          <button
                            key={val}
                            onClick={() =>
                            ...
                            className={`flex-1 aspect-square max-h-16 flex items-center justify-center text-xl font-bold rounded-xl border-2 transition-all ${
                              isSelected
                                ? "border-transparent ring-2 shadow-md scale-105"
                                : "border-black/10 hover:border-black/20 hover:bg-black/5 hover:scale-105"
                            }`}
                            style={{
                              backgroundColor: isSelected
                                ? "var(--c-primary)"
            ...
            {currentQuestionIndex === questions.length && (
              ...
              >
                ...
                >
                  {score}{" "}
                  <span className="text-2xl font-semibold opacity-50 tracking-normal text-gray-400">
                    pts
                  </span>
                </div>
