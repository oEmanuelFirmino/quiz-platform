...
import { doc, getDoc, updateDoc, collection, getDocs, addDoc, deleteDoc, writeBatch } from "firebase/firestore";
import { useAuth } from "../../AuthContext";
import { Save, Plus, Trash2, ArrowLeft, GripVertical, Settings2, Eye, BarChart3, Download, ArrowUp, ArrowDown } from "lucide-react";
...
export default function Editor() {
  ...
  const [activeTab, setActiveTab] = useState<'editor' | 'settings' | 'results'>('editor');
    ...
    const loadCode = async () => {
      try {
        ...
        if (qDoc.exists()) {
          ...
          const qs = qSnapshot.docs.map(d => ({ qid: d.id, ...d.data() })).sort((a: any, b: any) => a.order - b.order);
          setQuestions(qs);
        } else {
          navigate("/admin");
        }
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      // Simple sync: delete all and recreate for questions to guarantee correct order/updates easily 
      // in a real app better to diff, but for simplicity we fetch existing, delete removed, add new, update existing.
      ...
      const newQSnapshot = await getDocs(collection(db, "quizzes", id!, "questions"));
      const qs = newQSnapshot.docs.map(d => ({ qid: d.id, ...d.data() })).sort((a: any, b: any) => a.order - b.order);
      setQuestions(qs);
  ...
  const updateOptionPoints = (qIndex: number, oIndex: number, points: number) => {
    const q = [...questions];
  ...
  const addOption = (qIndex: number) => {
    const q = [...questions];
    q[qIndex].options.push({ id: Math.random().toString(36).substr(2, 9), text: "", points: 0 });
    setQuestions(q);
  };
  ...
  const loadSubmissions = async () => {
    ...
    try {
      const subSnapshot = await getDocs(collection(db, "quizzes", id!, "submissions"));
    ...
    <div className="flex flex-col h-full">
      <div className="flex items-center justify-between mb-8 pb-4 border-b border-gray-200">
        <div className="flex items-center gap-4">
          <button onClick={() => navigate('/admin')} className="p-2 -ml-2 rounded-lg text-gray-500 hover:bg-gray-100"><ArrowLeft className="w-5 h-5"/></button>
          <input 
            type="text" 
            value={quiz.title} 
        ...
        <div className="flex-1 pb-24">
          {activeTab === 'editor' && (
            <div className="space-y-6">
              {questions.map((q, qIndex) => (
                <div key={qIndex} className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 group">
                  <div className="flex justify-between items-start mb-4">
                    <div className="flex items-center gap-2 flex-1">
                      <span className="text-gray-400 font-medium text-sm">#{qIndex + 1}</span>
                      <input 
                        type="text" 
                        value={q.text}
                    ...
                    <div className="flex items-center gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                      <button onClick={() => moveQuestionUp(qIndex)} disabled={qIndex === 0} className="text-gray-400 hover:text-indigo-600 p-2 disabled:opacity-30 disabled:hover:text-gray-400">
                        <ArrowUp className="w-4 h-4" />
                      </button>
                      <button onClick={() => moveQuestionDown(qIndex)} disabled={qIndex === questions.length - 1} className="text-gray-400 hover:text-indigo-600 p-2 disabled:opacity-30 disabled:hover:text-gray-400">
                        <ArrowDown className="w-4 h-4" />
                      </button>
                      <button onClick={() => removeQuestion(qIndex)} className="text-gray-400 hover:text-red-500 p-2 ml-2">
                  ...
                  <div className="pl-6 space-y-3 mt-6">
                    ...
                    <button onClick={() => addOption(qIndex)} className="text-sm font-medium text-indigo-600 mt-2 hover:text-indigo-800 flex items-center gap-1.5 ml-7">
              ...
              >
                ...
                <span className="font-medium text-lg">Adicionar nova pergunta</span>
              </button>
            </div>
          )}
          ...
          {activeTab === 'settings' && (
             <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
                <div>
                  <label className="block text-sm font-medium text-gray-700 mb-1">Descrição / Instruções <span className="text-gray-400 font-normal text-xs">(Aparece na tela inicial)</span></label>
                  <textarea 
                ...
                <div className="grid grid-cols-2 gap-6 pt-6 border-t border-gray-100">
                  <div>
                    ...
                    <input 
                      ...
                      onChange={(e) => setQuiz({...quiz, theme: {...quiz.theme, logoUrl: e.target.value}})}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Cor Primária (Card selecionado)</label>
                    <div className="flex gap-2">
                       <input 
                        type="color" 
                        value={quiz.theme?.primaryColor || '#4f46e5'} 
                        onChange={(e) => setQuiz({...quiz, theme: {...quiz.theme, primaryColor: e.target.value}})}
                  ...
                  <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Cor do Botão Principal</label>
                    <div className="flex gap-2">
                       <input 
                        type="color" 
                        value={quiz.theme?.buttonColor || '#4f46e5'} 
                        onChange={(e) => setQuiz({...quiz, theme: {...quiz.theme, buttonColor: e.target.value}})}
                        className="h-9 w-12 rounded border border-gray-200 p-0.5"
                      />
                       <input 
                        type="text" 
                   ...
                   <div>
                    <label className="block text-sm font-medium text-gray-700 mb-1">Cor de Fundo (Página)</label>
                    <div className="flex gap-2">
                       <input 
                        type="color" 
                        value={quiz.theme?.bgColor || '#f9fafb'} 
                        onChange={(e) => setQuiz({...quiz, theme: {...quiz.theme, bgColor: e.target.value}})}
                        className="h-9 w-12 rounded border border-gray-200 p-0.5"
                      />
                       <input 
                        type="text" 
                ...
                <div className="pt-6 border-t border-gray-100 flex items-center justify-between">
                  <div className="flex items-center gap-4">
                     <div className="w-16 h-16 rounded-xl flex items-center justify-center border shadow-sm" style={{ backgroundColor: quiz.theme?.bgColor || '#f9fafb' }}>
                        <div className="w-8 h-8 rounded shrink-0" style={{ backgroundColor: quiz.theme?.buttonColor || '#4f46e5' }}></div>
                     </div>
                     <div>
                       <span className="block text-sm font-medium bg-gray-50 px-2 py-0.5 rounded text-gray-600 uppercase tracking-wide">Preview de cores</span>
                       <span className="block text-xs text-gray-500 mt-1">Como vai ficar no quiz</span>
                     </div>
                  </div>
                </div>

                <div className="pt-6 border-t border-gray-100">
                  <h4 className="text-sm font-medium text-gray-700 mb-4">Mensagens Personalizadas de Resultado</h4>
                  <div className="space-y-3">
                    {(quiz.scoreMessages || []).map((msg: any, i: number) => (
                      <div key={i} className="flex gap-3 items-end">
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Mín (pontos)</label>
                          <input type="number" value={msg.minScore} onChange={e => {
                        ...
                        <div>
                          <label className="block text-xs text-gray-500 mb-1">Máx (pontos)</label>
                          <input type="number" value={msg.maxScore} onChange={e => {
                            const newMsgs = [...(quiz.scoreMessages || [])];
                        ...
                        <div className="flex-1">
                          <label className="block text-xs text-gray-500 mb-1">Mensagem a ser exibida</label>
                          <input type="text" value={msg.message} onChange={e => {
                            const newMsgs = [...(quiz.scoreMessages || [])];
                            newMsgs[i].message = e.target.value;
                            setQuiz({...quiz, scoreMessages: newMsgs});
                          }} className="w-full border-gray-300 rounded-lg shadow-sm focus:border-indigo-500 focus:ring-indigo-500 sm:text-sm p-2 border" placeholder="Ex: Parabéns! Excelente pontuação!" />
                        </div>
                        <button type="button" onClick={() => {
                          const newMsgs = [...(quiz.scoreMessages || [])];
                          newMsgs.splice(i, 1);
                          setQuiz({...quiz, scoreMessages: newMsgs});
                        }} className="p-2 mb-0.5 text-gray-400 hover:text-red-500 rounded-lg hover:bg-red-50"><Trash2 className="w-4 h-4" /></button>
                      </div>
                    ))}
                    <button type="button" onClick={() => {
                      const newMsgs = [...(quiz.scoreMessages || []), { minScore: 0, maxScore: 10, message: "" }];
                      setQuiz({...quiz, scoreMessages: newMsgs});
                    }} className="text-sm font-medium text-indigo-600 mt-2 hover:text-indigo-800 flex items-center gap-1.5"><Plus className="w-3.5 h-3.5" /> Adicionar regra de mensagem</button>
                  </div>
                </div>
             </div>
          )}
          ...
          {activeTab === 'results' && (
            <div className="bg-white rounded-xl border border-gray-200 shadow-sm p-6 space-y-6">
              <div className="flex justify-between items-center mb-6 border-b pb-4">
                <h3 className="text-lg font-semibold text-gray-900">Métricas do Quiz</h3>
                {submissions.length > 0 && (
                  <button 
                    onClick={() => downloadCSV(submissions, `Respostas - ${quiz.title}`)}
              ...
              {loadingSubmissions ? (
                <div className="text-gray-500 text-center py-6">Carregando resultados...</div>
              ) : submissions.length === 0 ? (
                <div className="text-gray-500 text-center py-6">Ainda não há respostas para este quiz.</div>
              ) : (
                <div className="space-y-6">
                  <div className="grid grid-cols-2 gap-4">
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                      <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Total de Respostas</div>
                      <div className="text-3xl font-bold text-gray-900">{submissions.length}</div>
                    </div>
                    <div className="bg-gray-50 p-4 rounded-xl border border-gray-100">
                      <div className="text-xs font-semibold text-gray-500 uppercase tracking-wider mb-1">Média de Pontos</div>
                      <div className="text-3xl font-bold text-gray-900">
                        {Math.round(submissions.reduce((acc, curr) => acc + curr.totalScore, 0) / submissions.length * 100) / 100}
                      </div>
                    </div>
                  </div>
                          ...
                          {submissions.slice().reverse().map(sub => (
                            <tr key={sub.id}>
                              <td className="px-6 py-4 whitespace-nowrap text-sm text-gray-500">
                                {sub.submittedAt && sub.submittedAt.toDate ? sub.submittedAt.toDate().toLocaleString() : '-'}
                              </td>
